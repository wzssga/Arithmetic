...
public class niu10 {
	 ...
	 * 来源：牛客网
	 * 
	 * 如果一个整数不为0，那么这个整数至少有一位是1。如果我们把这个整数减1，那么原来处在整数最右边的1就会变为0，原来在1后面的所有的0都会变成1
	...
	private static int NumberOf1_low(int n) {
		int count = 0;
		int flag = 1;
		while (flag != 0) {//一直循环到超出int范围
			if ((n & flag) != 0) {//&运算不等0，说明n的这一位是1
				count++;
			}
			flag = flag << 1;//flag*2，也就是说flag的1位向左移一位，这样操作flag中永远只有，达到了遍历n二进制每一位的效果 
		}
		return count;
	}
}
